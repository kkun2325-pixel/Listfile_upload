...
import { getNextListNumber } from "@/lib/db";
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, next_number: nextNumber });
  } catch (error) {
    console.error("Next number error:", error);
    return NextResponse.json({ success: false, message: "採番取得中にエラーが発生しました" }, { status: 500 });
  }
}
