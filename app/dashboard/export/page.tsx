  ...
  async function handleZipExport() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setExportErrorDetail(err.detail ?? "");
        return;
      }
      ...
      {exportError && (
        <div className="bg-red-50 border border-red-200 rounded-lg mb-6 overflow-hidden">
          <div className="flex items-start justify-between px-4 py-3">
            ...
            <button
              onClick={() => { setExportError(""); setExportErrorDetail(""); setShowErrorDetail(false); }}
              className="text-red-300 hover:text-red-500 ml-3 shrink-0 text-lg leading-none"
            >
              ×
            </button>
          </div>
          {showErrorDetail && exportErrorDetail && (
            <div className="px-4 pb-4 border-t border-red-100">
              <div className="mt-3 bg-red-100 rounded p-3 font-mono text-xs text-red-900 break-all whitespace-pre-wrap max-h-48 overflow-y-auto">
                {exportErrorDetail}
              </div>
              <button
                onClick={() => navigator.clipboard.writeText(exportErrorDetail)}
